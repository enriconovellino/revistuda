...
export class TurmaRepository implements ITurmaRepository {
  ...
  async create(turma: Turma): Promise<Turma> {
    ...
    return new Turma(created.turma_id, created.nome_turma, created.descricao_turma ?? undefined, created.capacidade_maxima ?? undefined);
  }

  async findAll(): Promise<Turma[]> {
    const turmas = await this.prisma.turma.findMany();
    return turmas.map((t) => new Turma(t.turma_id, t.nome_turma, t.descricao_turma ?? undefined, t.capacidade_maxima ?? undefined));
  }

  async findById(id: number): Promise<Turma | null> {
    const t = await this.prisma.turma.findUnique({ where: { turma_id: id } });
    if (!t) return null;
    return new Turma(t.turma_id, t.nome_turma, t.descricao_turma ?? undefined, t.capacidade_maxima ?? undefined);
  }
