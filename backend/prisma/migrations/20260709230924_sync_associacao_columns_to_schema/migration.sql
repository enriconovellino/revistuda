...
-- revertendo a divergência introduzida por align_associacao_er_diagram de forma segura e idempotente.
...
BEGIN
    IF EXISTS (
        ...
        WHERE table_name = 'AssociacaoCorreta' 
          AND column_name = 'item_origem_id'
    ) THEN
...
BEGIN
    IF EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_name = 'RespostaAssociacao' 
          AND column_name = 'item_origem_id'
    ) THEN
...
BEGIN
    IF EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_name = 'TentativaAssociacao' 
